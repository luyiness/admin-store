    ...
    <div class="content-wrapper">
        ...
        <section class="content" style="padding-left: 0px;">
            <div class="infoC">
                        ...
                        <div class="infoListDetils proListCon">
                            <div class="proListC h">
                                <div class="detilsList">
                                    <label><em class="red">*</em>产品名称：</label>
                                    <input type="text"
                                            <#if remark == 'add'>
                                            <#elseif remark == 'edit'>
                                                disabled="disabled"
                                           ...
                                           id="modelName" maxlength="50" placeholder="">
                                </div>
                                <div class="detilsList">
                ...
                <div class="infoList goodsInfo" id="goodsInfo" style="overflow: hidden;display: none;">
                    <div class="goods_modal"></div>
                    <p class="infoTit">图片管理</p>
                    <div class="infoListDetils">
                        <div class="detilsList imgList file-uploads-group"><label style="vertical-align: top;"><em
                                        class="red">*</em>商品图片：</label>
                            <div class="proPicCon">
                ...
                <div class="infoList goodsInfo" style="display: none;" id="infoDetail">
                ...
                <a href="javascript:history.go(-1)"
                   style="display: inline-block;margin-right: 30px; border: 1px solid #199eda;" onclick="goBack()"
                   class="goList">返回上一页</a>
                <div class="submitBtn" style="margin: 0 auto;text-align:center; width: 50%;margin-bottom: 30px;">
                    <a href="javascript:history.go(-1)"
                       style="display: inline-block;margin-right: 30px; border: 1px solid #199eda;" onclick="goBack()"
                    ...
                    <div style="display: inline-block;margin-right: 30px;color: #199eda; border: 1px solid #199eda;background: transparent;"
                         class="reChoice " onclick="ProductEditFunc.setGoodsDatas()">暂存商品
                    </div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <div class="reChoice" style="display: inline-block" onclick="saveProduct()">提交产品</div>
                </div>
            </div>
...
<script type="text/javascript"
        src="${p_static}/admin/js/goods/addProduct.js"></script>
...
<script id="productBrand_" type="text/html">
    ...
    <#elseif remark == 'edit'>
        {{if data}}
        {{each data as brand i}}
        <option value="{{brand.id}}" selected="selected" disabled="disabled">{{brand.name}}</option>
        {{/each}}
        {{/if}}
    <#elseif remark == 'productView'>
        {{if data}}
    ...
    <div class="detilsList">
        ...
        <div class="standardValueDiv" style="width: 65%;display: inline-block;vertical-align: top;">
            ...
            <span style="margin-bottom: 10px;display: inline-block;vertical-align: middle;width: 48%;cursor: pointer;">
                <input class="standardValue" onchange="lockStandardInput(this);" type="checkbox"
                        <#if remark == 'add'>
                        <#elseif remark == 'edit'>
                            disabled="disabled"
                        <#elseif remark == 'productView'>
...
//                alert(text);
    ...
    var ProductEditFunc = {
        ...
        setGoodsDatas: function () {
            ...
            for (var i = 0; i < picPaths.length; i++) {
                ...
                picTmp.compressPath = picPaths[i].compressFilePath;
                picTmp.type = "0";
                picTmp.orderSort = i;
                pics[i] = picTmp;
            }
        ...
        goodsDataValidate: function (showPop) {
            ...
            if (goodsName == undefined || goodsName == "") {
                ...
                //不要放开这里了
            }/*else if(!(/^[a-zA-Z0-9\u4e00-\u9fa5]{1,15}$/).test(goodsName)){
            // /^[\u4e00-\u9fa5_a_zA_Z0-9]{0,14}+$/
            //(!(/^[A-Za-z\u4e00-\u9fa5]+$/).test(goodsName))
                showMessage({message: "商品名称只能输入中文、英文、数字,且长度不超过15位"});
                return false;
            }*/
            ...
            for (var i = 0; i < tableDatas.length; i++) {
                ...
                for (var j = 0; j < data.length; j++) {
                    if (j == 0) {
                        table = table +
                            "<td>" +
                            "<input class='tableStandard' type='checkbox'" +
                            <#if remark == 'add'>
                            <#elseif remark == 'edit'>
                            "disabled ='disabled'" +
                            "checked = 'checked'" +
                    ...
                    if (j == data.length - 1) {
                        <#if remark == 'add'>
                        table = table + "<td><a href='javascript:void(0)' class='editGoods' onclick = 'editButton(\"" + data[j] + "\")'>" + category + "</a></td>";
                        <#elseif remark == 'edit'>
                        <#elseif remark == 'productView'>
                        </#if>
                    } else {
    ...
    function saveProduct() {
        ...
        if (!goodsInfoIsHide) {
            showMessage({message: "请先暂存商品，商品暂存后方可提交保存"});
            return false;
        }
    ...
    document.onkeydown = function (e) {
        ...
        if (ev.keyCode == 8 && document.activeElement.id == "infoDetail") {
            return false;
        }
        if (ev.keyCode == 46 && document.activeElement.id == "infoDetail") {
            return false;
        }
    }
    </#if>
